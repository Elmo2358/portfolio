...
### Production Workflow（本番環境での検証）

- 本番デプロイ後は、実際の本番環境のパフォーマンスと機能を検証してください
- ローカルでの改善が本番環境に反映されると仮定しないでください
- 以下を確認してください：
  - 読み込み時間のテスト
  - 機能の動作確認
  - ビルドキャッシュなど本番環境固有の問題のチェック
- ローカルでの改善が本番環境で十分でない場合、本番固有の要因を調査してください
...
- ルーティング、フィルタリング、データ表示ロジックを変更した後は、コンポーネントレベル AND そのコンポーネントが呼び出されている場所の両方で実装を検証してください
- Grepを使用してコンポーネントの全使用箇所を検索し、変更が正しく伝播していることを確認してください
- 実際のデータでテストし、変更がコールチェーン全体を通じて正しく動作することを確認してください
...
- 既存のUIコンポーネントと統合する機能を実装する場合（例：ダッシュボードにカードを追加）、ユーザーが以下のどれを望んでいるか明確にしてください：
  1. 統一された/グループ化された表示
  2. 個別のアクセス
  3. 両方のオプション
- 明示的な確認なしに、既存のアクセスパスを削除しないでください
- 実装前にUI構造のモックアップや説明を提示してください
...
- データベーススキーマ変更やデータ永続化作業を行う場合、以下を明示的に検証してください：
  1. Prismaクライアントが再生成されている（`pnpm prisma generate`）
  2. マイグレーションが適用されている（`pnpm prisma migrate deploy`）
  3. 必要に応じてキャッシュがクリアされている
  4. サーバー再起動後もデータが永続化されている
- これらの検証が完了するまで、データベース関連タスクを完了とみなさないでください
...
- 国際化されたコンテンツ（特に日本語）を扱う場合は、常にバッククォートでテンプレートリテラルを使用し、文字列補間を検証してください
- テンプレート文字列内で引用符の種類を混在させないでください。構文エラーを防ぐため、一貫性を保ってください
