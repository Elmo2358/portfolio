...
interface BucketListItem {
  id: string
  title: string
  category: "travel" | "experience" | "goal" | "other"
...
export function BucketListManager() {
  ...
  // ステータス変更
  const handleStatusChange = async (item: BucketListItem, newStatus: BucketListItem["status"]) => {
    try {
  ...
  // ステータスバッジ
  const getStatusBadge = (status: BucketListItem["status"]) => {
    const statusConfig = {
      ...
      <div className="flex flex-wrap gap-4">
        ...
        <div className="flex gap-1 rounded-lg border border-emerald-500 bg-emerald-50 p-1 dark:bg-emerald-950 dark:border-emerald-600">
          {(["all", "travel", "experience", "goal", "other"] as CategoryFilter[]).map((filter) => (
            ...
            >
              {filter === "all" ? "すべて" : categoryInfo[filter as keyof typeof categoryInfo].label}
            </button>
          ))}
        </div>
      </div>
                    ...
                    <div className="flex gap-1">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          const statusCycle: Record<BucketListItem["status"], BucketListItem["status"]> = {
                            planning: "in_progress",
                            in_progress: "completed",
                            completed: "planning"
                          }
                          handleStatusChange(item, statusCycle[item.status])
                        }}
