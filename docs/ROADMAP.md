...
| 機能 | ステータス | 説明 |
|------|----------|------|
| CLI統合 | 🔲 | uec-portal-cliとの連携 |
| お知らせ取得 | 🔲 | 掲示板情報をDBキャッシュ |
| 予定取得 | 🔲 | 今週の予定をDBキャッシュ |
| 時間割取得 | 🔲 | 授業時間割をDBキャッシュ |
| 定期同期 | 🔲 | 2時間ごとの自動同期 |
| 手動更新 | 🔲 | ボタンクリックで最新取得 |

**詳細計画**: [`.claude/plans/tranquil-twirling-summit.md`](../../.claude/plans/tranquil-twirling-summit.md) を参照

**技術スタック**:
- uec-portal-cli（Playwrightベース）
- Prismaキャッシュモデル
- Next.js API Routes
- Vercel Cron（定期実行）

**注意点**: Vercel Serverless FunctionsではPlaywrightが動作しないため、ローカル/別サーバー（Railway等）でのCLI実行が必要
